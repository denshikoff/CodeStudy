...
type
  ...
  LinkedList = class
    private
      list:TList;
    public
      procedure Add(info:TInfo);  //добавление элемента
      function Contains(info:TInfo): boolean; //проверка на наличие элемента
      procedure search(info:TInfo);    //поиск и удаление элемента
      procedure Clear();    //очистить список
      function ToString(): string; //создание строки
      function Get(index:integer): TInfo;    //получение элемента
      constructor create();
...
implementation
 ...
 function InfoToString(info: TInfo):string;
	begin
		result := 'Деталь с ID № '+ IntToStr(info.getId) + ' Цена ' +
     IntToStr(info.getPrice) + ' Тип детали: ' + info.getType + #10+#13;
	end;
